...
import { Menu, Search, ChevronDown, Package, DollarSign, Clock, CheckCircle, FileText, Box, Printer, Ship, MapPin, Calendar, Users, ArrowRight, AlertTriangle, ShieldAlert, Download, FileSpreadsheet, User } from 'lucide-react';
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from 'recharts';
import { useAuth } from '@/lib/auth-context';

function StatCard({ icon: Icon, title, value, subtitle, positive = true }: any) {
  return (
    <div className="border border-[#296341]/20 lg:border-[#296341] bg-white p-4 lg:p-6 flex items-center gap-4 shadow-sm hover:shadow-md lg:hover:shadow-none transition-shadow">
      <div className="w-12 h-12 lg:w-11 lg:h-11 bg-[#296341]/10 lg:bg-transparent flex items-center justify-center flex-shrink-0">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className={`text-[11px] lg:text-[16px] font-bold px-2 lg:px-0 py-0.5 lg:py-0 inline-block lg:block ${
...
function ShipmentTable({ title, badge, shipments, showCount }: any) {
  return (
    <div className="border border-[#296341]/20 lg:border-[#296341] bg-white p-4 lg:p-6 mb-8 shadow-sm lg:shadow-none">
      <div className="flex items-center justify-between mb-6 gap-4">
        ...
        <div className="flex items-center gap-2 lg:gap-4 flex-shrink-0">
          <div className="px-3 lg:px-6 py-1 lg:py-2 text-white text-lg lg:text-[30px] font-black tracking-tighter bg-[#296341]">
            {badge}
          </div>
          <button className="p-2 lg:p-0 bg-gray-50 lg:bg-transparent hover:bg-gray-100 lg:hover:bg-transparent transition-all active:scale-95 text-[#296341]">
      ...
      <div className="lg:hidden space-y-3">
        {shipments.length === 0 ? (
          ...
          shipments.map((shipment: any, idx: number) => {
            ...
            return (
              <div key={idx} className="bg-white/50 border border-gray-100 p-4">
                  <div className="flex items-center justify-between border-b border-gray-100 pb-3 mb-3">
                    <div className="flex items-center gap-3">
                      <div className="bg-[#296341] text-white px-3 py-1 text-sm font-black italic">#{invoiceNo}</div>
                      <div className={`px-2 py-0.5 text-[10px] font-black uppercase tracking-widest border ${isPaid ? 'border-green-200 text-green-700 bg-green-50' : 'border-red-100 text-red-600 bg-red-50'}`}>
                        {status}
                      </div>
                    </div>
      ...
      <div className="flex flex-col lg:flex-row items-center justify-between mt-6 gap-4 lg:gap-0 border-t lg:border-none border-gray-100 pt-6 lg:pt-0">
        ...
        <Link
          href={`/admin/dashboard/shipments?type=${badge}`}
          className="w-full lg:w-auto bg-[#296341] lg:bg-transparent border-2 border-[#296341] text-white lg:text-[#296341] px-8 py-3.5 lg:py-2 text-[15px] font-black hover:bg-[#1e4c30] lg:hover:bg-[#296341] lg:hover:text-white transition-all transform active:scale-95 shadow-xl lg:shadow-none shadow-[#296341]/20 text-center block"
        >
          View all Shipments
        </Link>
      </div>
...
export default function App() {
  ...
  const exportToCSV = () => {
    const shipments = data?.allRecentShipments;
    if (!shipments || shipments.length === 0) {
      alert('No shipment data available for the selected range.');
      return;
    }
    ...
    const rows = shipments.map((s: any) => [
      ...
      `${s.user?.firstName || ''} ${s.user?.lastName || ''}`.trim() || 'N/A',
      s.contactName || 'N/A',
      s.type || 'N/A',
      s.paymentStatus || 'N/A',
      s.totalAmount || 0,
    ...
    link.setAttribute('download', `dean_shipping_${range}_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const exportToPDF = () => {
    ...
    const rangeLabel = range === 'all' ? 'All Time' : range === 'today' ? 'Today' : range.toUpperCase();
    const stats = data?.stats || {};

    const htmlContent = `<!DOCTYPE html>
<html>
<head>
  <title>Dean's Shipping - Dashboard Report</title>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body { font-family: Arial, sans-serif; padding: 40px; color: #333; }
    h1 { color: #296341; font-size: 24px; margin-bottom: 4px; }
    .subtitle { color: #777; font-size: 12px; margin-bottom: 24px; }
    .stats-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 12px; margin-bottom: 24px; }
    .stat-box { border: 1px solid #296341; padding: 12px; }
    ...
    th { background: #d4e0d9; border: 1px solid #296341; padding: 8px; text-align: left; font-weight: bold; }
...
<body>
  ...
  <div class="stats-grid">
    <div class="stat-box">
      ...
      <div class="stat-value">${stats.totalShipmentsToday || 0}</div>
    </div>
    <div class="stat-box">
      <div class="stat-label">Cash Collected</div>
      <div class="stat-value">$${(stats.cashCollectedToday || 0).toLocaleString()}</div>
    </div>
    <div class="stat-box">
      <div class="stat-label">Total Revenue</div>
      <div class="stat-value">$${(stats.totalRevenueToday || 0).toLocaleString()}</div>
    </div>
    <div class="stat-box">
      <div class="stat-label">Pending Payments</div>
      <div class="stat-value">${stats.pendingPaymentCount || 0}</div>
    </div>
  </div>

  <h2 style="color:#296341;font-size:16px;margin-bottom:8px;">Shipment Records (${shipments.length})</h2>
  <table>
    <thead>
      <tr>
        <th>Invoice</th>
    ...
    <tbody>
      ${shipments.map((s: any) => `
        <tr>
          ...
          <td>${`${s.user?.firstName || ''} ${s.user?.lastName || ''}`.trim() || 'N/A'}</td>
          <td>${s.contactName || 'N/A'}</td>
          <td>${s.type || 'N/A'}</td>
          <td class="${s.paymentStatus === 'PAID' ? 'paid' : 'unpaid'}">${s.paymentStatus || 'N/A'}</td>
          <td>$${(s.totalAmount || 0).toLocaleString()}</td>
          <td>${s.invoice?.paymentMode || 'N/A'}</td>
        </tr>
      `).join('')}
    </tbody>
  </table>
  <div class="footer">Dean's Shipping Ltd &mdash; Confidential</div>
    ...
    <div className="bg-white">
      <main className="max-w-[1400px] mx-auto px-4 sm:px-8 py-4 sm:py-8">
        ...
        <div className="flex items-start justify-between mb-6">
          ...
          {/* User name + role top-right (dashboard only) */}
          {user && (
            <div className="flex items-center gap-3 flex-shrink-0">
              <div className="text-right">
                ...
                <p className="text-[11px] font-bold text-[#296341] uppercase tracking-widest">
                  {user.role}
                </p>
              </div>
              <div className="w-10 h-10 bg-[#296341] flex items-center justify-center text-white font-black text-sm">
        ...
        <div className="flex flex-wrap items-center gap-3 mb-10">
          {/* Range Filters — includes All Time */}
          ...
          <div className="flex items-center gap-2 ml-auto">
            <button 
              onClick={exportToCSV}
              className="flex items-center gap-2 bg-white border-2 border-[#296341] text-[#296341] px-5 py-2.5 text-sm font-black uppercase tracking-widest hover:bg-[#296341] hover:text-white transition-all active:scale-95 shadow-lg shadow-gray-200"
            >
              <FileSpreadsheet className="w-4 h-4" /> CSV
            </button>
            <button 
              onClick={exportToPDF}
              className="flex items-center gap-2 bg-[#296341] text-white px-5 py-2.5 text-sm font-black uppercase tracking-widest hover:bg-[#1e4c30] transition-all active:scale-95 shadow-lg shadow-[#296341]/30"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
          <StatCard
            icon={Package}
            title={`Total Shipment ${rangeLabel}`}
            value={stats.totalShipmentsToday || "0"}
            subtitle={range === 'all' ? 'All time total' : `${stats.shipmentsChange >= 0 ? '+' : ''}${stats.shipmentsChange || 0}% from prev. period`}
          />
          <StatCard
            icon={DollarSign}
            title={`Collected Cash ${rangeLabel}`}
            value={`$${(stats.cashCollectedToday || 0).toLocaleString()}`}
            subtitle={range === 'all' ? 'All time total' : `${stats.cashChange >= 0 ? '+' : ''}${stats.cashChange || 0}% from prev. period`}
          />
          <StatCard
            icon={DollarSign}
            title={`Total Revenue ${rangeLabel}`}
            value={`$${(stats.totalRevenueToday || 0).toLocaleString()}`}
            subtitle={range === 'all' ? 'All time total' : `${stats.revenueChange >= 0 ? '+' : ''}${stats.revenueChange || 0}% from prev. period`}
          />
          <StatCard
            icon={Clock}
              ...
              {revenueChartData.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={revenueChartData}>
                    ...
                    />
                    <Tooltip
                      contentStyle={{ border: '2px solid #296341', fontWeight: 'bold' }}
            ...
            <div className="space-y-4 overflow-y-auto max-h-[400px] pr-2 custom-scrollbar">
              {recentActivity.length === 0 ? (
                ...
                recentActivity.map((activity: any, idx: number) => (
                  <div key={idx} className="pb-4 border-b border-[#d4e0d9] last:border-0 hover:bg-gray-50 transition-colors p-2">
        ...
        {upcomingVoyages.length > 0 && (
          <div className="border border-[#296341] p-6 mb-8">
            ...
            <div className="space-y-3">
              {upcomingVoyages.map((voyage: any) => {
                ...
                return (
                  <div
                    key={voyage.id}
                    className="flex items-center gap-4 bg-[#e5f7f1] border border-[#296341]/30 px-5 py-4 hover:shadow-md transition-shadow"
                  >
                    <div className="flex-shrink-0 w-14 h-14 bg-[#296341] flex flex-col items-center justify-center text-white">
        ...
        {recentIncidents.length > 0 && (
          <div className="border-2 border-red-500/20 bg-red-50/10 p-6 mb-8 shadow-sm">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-6 gap-4">
              ...
              <div className="flex items-center gap-4">
                <div className="px-4 py-1 bg-red-600 text-white text-sm font-black uppercase tracking-wider">
                  High Priority
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {recentIncidents.map((incident: any) => {
                ...
                return (
                  <div key={incident.id} className="bg-white border border-red-100 p-4 shadow-sm hover:shadow-md transition-shadow cursor-pointer group">
                    <div className="flex justify-between items-start mb-3">
                      <div className={`px-3 py-1 text-[10px] font-black uppercase tracking-tighter ${severityColors[incident.severity] || severityColors.MEDIUM}`}>
                        {incident.severity}
                      </div>
                    ...
                    <div className="flex items-center justify-between mt-auto pt-3 border-t border-gray-50">
                      <div className="flex items-center gap-2">
                        <div className="w-6 h-6 bg-gray-100 flex items-center justify-center text-[10px] font-black">
